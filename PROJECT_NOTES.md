...
* The unified editor retains immutable audio-buffer helpers for trim/normalize/WAV encoding, applies only a tiny trim-end fade to avoid dulling drum attacks, previews with the selected layer's trim, and can create named user-sample assets without assigning them before the layer draft is applied. Shared Audio Edit/Beat Alignment modes and combined Save Copy/Replace-and-Apply actions remain planned for the next waveform-workflow phase.
